...
1. 为什么选择柯西分布而不是正态分布做为 Abduction Net 的输出?

我始终认为, 所有的 instance/use/sample/individual 具备共同的底层生成机制, 预测结果的之后用到所有的样本是一个我们偏好的先验知识, 重尾的柯西分布是符合这个先验知识的. 而正态分布的尾部衰减太快, 导致预测的时候容易只用到了少数的样本, 导致预测结果的方差太大.  总而言之, 底层逻辑是柯西分布的鲁棒性更强, 更加适合学习因果律.  

2. 为什么柯西分布的组合封闭性可以让我们轻松地使用极大似然估计来设计损失函数?

柯西分布的组合封闭性意味着, 柯西分布的组合仍然是柯西分布. 这意味着, 我们可以将柯西分布的组合看作是一个新的柯西分布, 然后使用极大似然估计来训练模型. 
